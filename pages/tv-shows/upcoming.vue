...
const fetchUpcomingMovies = async () => {
  ...
  const upcomingEndpoint = `https://api.themoviedb.org/3/discover/tv?api_key=348088421ad3fb3a9d6e56bb6a9a8f80&language=en-US&page=1&sort_by=popularity.${ selectedSort.value }&air_date.gte=${ inTenDays }&with_genres=${ selectedFilter.value }`;
...
<template>
  ...
  <div class="main-container flex flex-row p-5">
    <SideBarView :genres="{ action: '10759', comedy: '35' }" :page-title="'Up Coming'" :selected-sort="selectedSort" :selected-filter="selectedFilter" @update-sort="selectedSort = $event" @update-filter="selectedFilter = $event" />
